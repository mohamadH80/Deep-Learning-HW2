...
class Linear(object):
    ...
    def forward(x, w, b):
        ...
        # pass
        N = x.size(0)
        out = torch.mm(x.view(N, -1), w) + b # add bias to each sample
        # print(x.size())
        ######################################################################
        #                        END OF YOUR CODE                            #
        ######################################################################
    ...
    def backward(dout, cache):
        ...
        # pass
        N_samples = x.size(0)
        dw = torch.mm(x.view(N_samples, -1).T, dout) # update weights
        dx = torch.mm(dout, w.T).view(x.size()) # delta of previous layer
        v1 = torch.ones((N_samples, 1), dtype=b.dtype, device='cuda') # a vactor of ones
        db = torch.mm(dout.T, v1).view(b.size())
        ##################################################
        #                END OF YOUR CODE                #
        ##################################################
        return dx, dw, db
...
class ReLU(object):
    ...
    def backward(dout, cache):
        ...
        x[x > 0] = 1
        x[x < 0] = 0
        dx = x * dout # just pass activation derivative
...
class Linear_ReLU(object):
    ...
    def forward(x, w, b):
        ...
        out, xwb_cache = Linear.forward(x, w, b)
        before_activasion = out
        after_activasion, before_activasion_cache = ReLU.forward(before_activasion)
        cache = (xwb_cache, before_activasion_cache)
        out = after_activasion
    ...
    def backward(dout, cache):
        ...
        # Replace "pass" statement with your code
        # pass
        xwb_cache, before_activasion_cache = cache
        dout_before_activasion = ReLU.backward(dout, before_activasion_cache)
        dx, dw, db = Linear.backward(dout_before_activasion, xwb_cache)
...
def softmax_loss(x, y):
    ...
    probs /= torch.sum(probs, axis=1, keepdim=True)
    # Calculate loss
    loss = torch.sum( torch.log(probs[torch.arange(N_samples), y]) ) / N_samples
    
    dx = probs
    dx[torch.arange(N_samples), y] -= 1
    dx /= N_samples
    # dx -= 1
...
class TwoLayerNet(object):
    ...
    def __init__(self, input_dim=3*32*32, hidden_dim=100, num_classes=10,
        ...
        # Nothing more!
        w1 = torch.normal(0.0, weight_scale, dtype=dtype, size=(input_dim, hidden_dim)).to(device)
        w2 = torch.normal(0.0, weight_scale, dtype=dtype, size=(hidden_dim, num_classes)).to(device)
    ...
    def loss(self, X, y=None):
        ...
        out_hiddenLayer, cache = Linear_ReLU.forward(X, w1, b1)
        scores, cache_linear = Linear.forward(out_hiddenLayer, w2, b2)
        ...
        dx2, dw2, db2 = Linear.backward(dout, cache_linear)
        dw2 += 2 * self.reg * w2

        dx1, dw1, db1 = Linear_ReLU.backward(dx2, cache)
        dw1 += 2 * self.reg * w1
...
class FullyConnectedNet(object):
    ...
    def __init__(self, hidden_dims, input_dim=3*32*32, num_classes=10,
        ...
        # Replace "pass" statement with your code
        # pass
        layers_dims = [input_dim] + hidden_dims + [num_classes] # Concatenating layers
        for l in range(self.num_layers):
            w_i = torch.normal(0.0, weight_scale, dtype=dtype, size=(layers_dims[l], layers_dims[l+1]), device=device)
            b_i = torch.zeros(layers_dims[l+1], dtype=dtype, device=device)
            indw = 'W'+str(l+1)
            indb = 'b'+str(l+1)
            self.params[indw] = w_i
            self.params[indb] = b_i
    ...
    def loss(self, X, y=None):
        ...
        for l in range(self.num_layers):
            ...
            if l < self.num_layers - 1:
                scores, cache = Linear_ReLU.forward(scores, wl, bl)
            else:
                scores, cache = Linear.forward(scores, wl, bl)                
        ...
        for l in range(self.num_layers):
            indw = 'W'+str(l+1)
            wl = self.params[indw]
            loss += self.reg * torch.sum(wl ** 2)
        for l in range(self.num_layers):
            lb = self.num_layers -1 - l
            cache = all_cache[lb]
            if l == 0:
                dout, dw, db = Linear.backward(dout, cache)
                indw = 'W'+str(lb+1)
                wlb = self.params[indw]
                indb = 'b'+str(lb+1)
                blb = self.params[indb]  
                dw += 2 * self.reg * wlb
                grads[indw] = dw
                grads[indb] = db
            else:
...
def create_solver_instance(data_dict, dtype, device):
    ...
    # Replace "pass" statement with your code
    # pass
    solver = Solver(model, data_dict,device=device, update_rule=sgd, optim_config={'learning_rate': 25e-2}, lr_decay=0.99, num_epochs=100, batch_size=150)
...
def get_three_layer_network_params():
    ...
    ###############################################################
    weight_scale = 1.5e-1   # Experiment with this!
...
def get_five_layer_network_params():
    ...
    # model achieves 100% training accuracy within 20 epochs.      #
    ################################################################
    learning_rate = 1.5e-1  # Experiment with this!
...
# Inspired by 'https://www.kaggle.com/code/greenarrow2018/fully-connected/notebook'